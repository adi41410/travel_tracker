...
class PointsService {
  ...
  Future<UserPoints> getUserPoints(String userId) async {
    ...
    final recentEntries = recentEntriesResult.map((map) => PointsEntry.fromMap(map)).toList();
  ...
  Future<Map<String, dynamic>> getPointsStats(String userId) async {
    ...
    // Points this month
    final thisMonthStart = DateTime.now().subtract(Duration(days: DateTime.now().day - 1));
